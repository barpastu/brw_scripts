...
for feature in layer:
    if theme is not "dom": 
        ...
        if int(year)>2012 :
             ...
             cmd += path_lv95 + "/"
             #os.system(cmd)
             cmd = "cp " + os.path.join(path_lv95, "ausschnitt_"+outfileName_jpeg) 
             cmd += " " + path_lv95 + "/"
             #os.system(cmd)
    ...
    if theme is not "dom":
        try:
            ...
            gtif=gdal.Open(os.path.join(path_lv95,"difference","orig_"+outfileName_jpeg))
            ...
            cmd += "--calc=\"(absolute(A)<="+tolerance+")*0 + (absolute(A)>"+tolerance+")*1\" --NoDataValue=10"
            os.system(cmd)
            ...
            logger_error.error(os.path.join(path_lv95,"difference",outfileName_jpeg)+" kann nicht verglichen werden")
            cmd = "cp " + os.path.join(path_lv95, "ausschnitt_"+outfileName_jpeg)
            cmd += " " + os.path.join(path_lv95,"difference")
            os.system(cmd)
